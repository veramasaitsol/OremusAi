...
//
// Source of truth = `account_transactions` (the synced Zoho GL postings). Each
// posting row carries a `transaction_id` plus a `transaction_number` (the voucher
// no.). A single transaction_id can span several sub-vouchers (e.g. a customer
// payment #437 plus the invoice applications CPL1183 / CPL1193 it settles), so we
// group by (transaction_id, transaction_number) — each pair is one self-balancing
...
router.get('/', async (req, res) => {
  try {
    // Clients with integration_type='zoho' inherit the admin's connection.
    ...
    // ── Shared WHERE clause (Zoho rows only — exclude QB/Xero rows) ───────────
    let where    = "at.user_id = ? AND at.platform = 'zoho'";
    const params = [effectiveUid];

    if (req.orgId) { where += ' AND at.org_id = ?';          params.push(req.orgId); }
    ...
    const [typeRows] = await pool.execute(
      `SELECT DISTINCT at.transaction_type AS t
         FROM account_transactions at
        WHERE at.user_id = ? AND at.platform = 'zoho'
              ${req.orgId ? 'AND at.org_id = ?' : ''}
              AND at.transaction_type IS NOT NULL AND at.transaction_type <> ''
        ORDER BY t ASC`,
      req.orgId ? [effectiveUid, req.orgId] : [effectiveUid]
    );
