...
// import FeedbackPopup from '../components/ai/FeedbackPopup.jsx';
...
// function AiCard({ data, currency, hasRows, feedback, onRate }) {
...
//         </div>
//       ) : !analysis && !data?.sql ? (
//         // Nothing structured came back — show the raw payload so the card is
//         // never silently empty.
//         <pre className="text-[11px] text-navy-500 dark:text-navy-400 overflow-x-auto whitespace-pre-wrap break-words">
...
//                 title="Good answer"
//                 className={`w-8 h-8 rounded-lg grid place-items-center border transition-colors ${feedback?.rating === 'up'
...
//                   : 'border-navy-200/80 dark:border-navy-700 text-navy-400 hover:text-emerald-600 dark:hover:text-emerald-300 hover:bg-navy-100 dark:hover:bg-navy-800'
//                   }`}
//               >
//                 <ThumbsUp size={13} />
//               </button>
...
//                 title="Bad answer"
//                 className={`w-8 h-8 rounded-lg grid place-items-center border transition-colors ${feedback?.rating === 'down'
//                   ? 'border-red-300 dark:border-red-500/40 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-300'
//                   : 'border-navy-200/80 dark:border-navy-700 text-navy-400 hover:text-red-600 dark:hover:text-red-300 hover:bg-navy-100 dark:hover:bg-navy-800'
//                   }`}
//               >
//                 <ThumbsDown size={13} />
//               </button>
...
//   const upsert = (id, fn, bump = true) => {
//     setConvos((list) => {
//       const idx = list.findIndex((c) => c.id === id);
//       const base = idx >= 0 ? list[idx] : { id, title: '', ts: Date.now(), messages: [] };
//       const next = { ...base, ...fn(base), ...(bump ? { ts: Date.now() } : {}) };
//       if (!next.title) {
...
//     if (!q || loading) return;
//     setInput('');
//     setPopup(null); // a new question dismisses any pending feedback prompt
...
//   // Inline thumbs on an answer card → open the feedback popup pre-filled.
//   const openFeedback = (m, msgKey, question, rating = null) => {
//     setPopup({
//       requestId: msgKey,
//       question,
//       clientId: activeClient?.id ?? null,
//       generatedSql: m.data?.sql || '',
//       rating: m.feedback?.rating || rating || null,
...
//     const { requestId, rating, correction, question, clientId, generatedSql } = popup;
//     await submitFeedback({ requestId, question, correction, clientId, generatedSql, rating });
//     if (popup.convoId) {
//       const { convoId } = popup;
//       upsert(convoId, (c) => ({
//         ...c,
//         messages: c.messages.map((m, i) => (
//           (m.id || `${convoId}_${i}`) === requestId
//             ? { ...m, id: m.id || requestId, feedback: { rating, correction } }
//             : m
//         )),
//       }), false); // don't bump the conversation timestamp for a rating
//     }
...
//                 className={`w-full text-left rounded-xl px-3 py-2.5 transition-colors border ${c.id === activeId
...
//                           data={m.data}
//                           currency={currency}
//                           hasRows={Array.isArray(m.data?.data) && m.data.data.length > 0}
//                           feedback={m.feedback}
//                           onRate={(rating) => openFeedback(
//                             m,
//                             m.id || `${activeId || 'c'}_${i}`,
...
//       {/* Post-answer feedback popup */}
//       {popup && (
//         <FeedbackPopup
//           popup={popup}
//           onRate={(rating) => setPopup((p) => ({ ...p, rating }))}
//           onCorrection={(correction) => setPopup((p) => ({ ...p, correction }))}
...
// One AI reply card: answer text, table, bars, SQL toggle, provenance footer.
function AiCard({ data, currency, hasRows, feedback, onRate, htmlContainerRef }) {
  const [copied, setCopied] = useState(false);
  const text = data?.answer ?? data?.response ?? data?.message ?? data?.text;
  const rows = Array.isArray(data?.data) ? data.data : null;
  const analysis = rows && rows.length ? analyze(rows) : null;
  console.log("AiCard data:", data, "analysis:", analysis, "hasRows:", hasRows);
  const copy = async () => {
    try {
      await navigator.clipboard.writeText(answerPlainText(data));
      setCopied(true);
...
export default function Analytics() {
  ...
  const send = async (raw, keepInput = false) => {
    const q = String(raw ?? input).trim();
    if (!q || loading) return;
  ...
  return (
    <div className="flex h-[calc(100vh-56px)] overflow-hidden">
      ...
      <section className="flex-1 min-w-0 flex flex-col">
        ...
        <div
          ref={scrollRef}
          className="flex-1 overflow-y-auto scroll-thin px-4 lg:px-8 py-6"
          onClick={(event) => {
            const button = event.target.closest('button.clarify-option');
            ...
            console.log('Clarification button clicked:', searchValue);

            setInput(searchValue);
            send(searchValue, true);
          }}
        >
